...
import { ChainReads, type ClosetEntry } from "./chain/reads.js";
...
export interface AppContext {
  ...
  reads: ChainReads;
  auth: AuthService;
  /** wallet(lowercase) → closet entries; TTL = config.assetsCacheTtlSeconds. */
  assetsCache: TtlCache<ClosetEntry[]>;
...
export function createAppContext(config: AppConfig, db: Db, chain: ChainContext): AppContext {
  ...
  const assetsCache = new TtlCache<ClosetEntry[]>(config.assetsCacheTtlSeconds * 1000);

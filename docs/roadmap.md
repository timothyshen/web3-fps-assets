...
- **Unity 不碰私钥、不连链。** 安全底线。
- **Web 应用和游戏用同一套账号，账号为父、钱包为子。** 已确认（2026-08-08）：
  登录、奖励、权益一律锚定账号（playerId），钱包是账号的后绑定子属性，可更换
  （re-bind 流程为生产 TODO）。当前实现与此一致（后端 playerId↔wallet 绑定、
  奖励锚定 playerId）；Web 端暂以"钱包直连看衣柜"作 demo 便利，账号登录视图
  是后续项，不改变父子关系。
